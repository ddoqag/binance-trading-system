...
class BacktestEngine:
    ...
    def run(self, df, strategy):
        ...
        # 关键修复：显式创建float类型的列
        df['holdings'] = pd.Series(0.0, index=df.index, dtype=float)
        df['cash'] = pd.Series(float(self.initial_capital), index=df.index, dtype=float)
        df['total_value'] = pd.Series(float(self.initial_capital), index=df.index, dtype=float)
        df['returns'] = pd.Series(0.0, index=df.index, dtype=float)
        ...
        for i in range(1, len(df)):
            ...
            if signal == 1 and position == 0:  # 买入
                ...
                shares = cash_available * (1 - self.commission) / price
                cost = shares * price
                commission_fee = cost * self.commission
                total_cost = cost + commission_fee

                df.loc[date, 'cash'] = cash_available - total_cost
                ...
                df.loc[date, 'cash'] = df['cash'].iloc[i-1] + net_revenue
            ...
            # 计算总价值
            current_holdings = df.loc[date, 'holdings']
            current_cash = df.loc[date, 'cash']
            df.loc[date, 'total_value'] = current_cash + current_holdings * price

            # 计算收益率
            prev_value = df['total_value'].iloc[i-1]
            curr_value = df.loc[date, 'total_value']
            df.loc[date, 'returns'] = (curr_value - prev_value) / prev_value if prev_value > 0 else 0
