...
start "GoEngine-LIVE-MARGIN" cmd /k "echo [LIVE MARGIN MODE - BTCUSDT] && echo [WARNING] 使用真实资金! && go run main_with_http.go btcusdt live margin || echo [ERROR] Go Engine crashed && pause"
...
start "Python-MVPTrader [LIVE]" cmd /k "echo [LIVE MODE - 实盘交易] && echo [WARNING] 使用真实资金! && python mvp_trader_live.py --symbol BTCUSDT || echo [ERROR] Python crashed && pause"
...
echo   curl http://127.0.0.1:8080/api/v1/status
echo.
