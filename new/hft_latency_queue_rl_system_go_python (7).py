...
# ======================================================
# LATENCY ENGINE (REALISTIC EXECUTION DELAY SIMULATION)
# ======================================================
...
class LatencyEngine:
    def __init__(self, base_latency=0.05, jitter=0.02):
        self.base_latency = base_latency
        self.jitter = jitter
        self.event_queue = []  # min-heap (execution_time, event)

    def sample_latency(self):
        return max(0.0, np.random.normal(self.base_latency, self.jitter))

    def submit_order(self, order):
        delay = self.sample_latency()
        exec_time = time.time() + delay
        heapq.heappush(self.event_queue, (exec_time, order))
    ...
    def process(self):
        ...
        while self.event_queue and self.event_queue[0][0] <= now:
            _, order = heapq.heappop(self.event_queue)
            executed.append(order)

        return executed

# ======================================================
# LATENCY-AWARE ENVIRONMENT
# ======================================================

class LatencyAwareEnv(LiveTradingEnvFull):
    def __init__(self, depth_feed, trade_feed):
        super().__init__(depth_feed, trade_feed)
    ...
    def step(self, action):
        ...
        if -0.5 < action < 0.5:
            ...
            self.latency_engine.submit_order(order)

        elif action >= 0.5:
            order = {"type": "market", "side": "buy", "size": 1.0}
            self.latency_engine.submit_order(order)

        elif action <= -0.5:
            order = {"type": "market", "side": "sell", "size": 1.0}
            self.latency_engine.submit_order(order)

        # 2. Process delayed executions
        executed_orders = self.latency_engine.process()

        pnl = 0
        for o in executed_orders:
            if o["type"] == "market":
                fills = self.engine.match_market(o["side"], o["size"])
                for price, size in fills:
                ...
                self.queue_tracker.place_order(
                    o["id"], o["side"], o["price"], o["size"], orderbook_snapshot
                )

        # 3. Update queue dynamics
        trade_price = self.feed.best_bid
        trade_size = abs(np.random.randn())
        ...
        # 4. State construction (DELAYED perception)
        features = self.feature_engine.get_features()
        ...
        state = np.append(features, queue_ratio)

        # 5. Reward (latency-aware)
        latency_penalty = self.latency_engine.base_latency
        reward = pnl - 0.2 * latency_penalty - 0.1 * queue_ratio

        return state, reward, False, {}

# ======================================================
# RESULT:
# - Orders are delayed before entering book
# - Execution uncertainty introduced
# - Queue position becomes realistic
# - SAC learns latency-aware strategies
# ======================================================

# ======================================================
# FINAL SYSTEM STATUS:
# You now have a FULL MICROSTRUCTURE RL SYSTEM:
# - L2 Depth (real)
# - Trade Flow (real)
# - Queue Position (realistic)
# - Matching Engine (exchange-like)
# - SAC (decision brain)
# - Latency Engine (real-world constraint)
# ======================================================