#!/usr/bin/env python3
"""
Qwen3.5-7B QLoRA 微调脚本 - RTX 3090 优化版
...
def parse_args():
    parser = argparse.ArgumentParser(description='Qwen3.5-7B QLoRA 微调 - RTX 3090 优化版')
    ...
    parser.add_argument('--max_seq_length', type=int, default=1536, help='最大序列长度 (RTX 3090 优化)')
    parser.add_argument('--lora_r', type=int, default=32, help='LoRA Rank (RTX 3090 优化)')
    parser.add_argument('--lora_alpha', type=int, default=64, help='LoRA Alpha (RTX 3090 优化)')
    parser.add_argument('--batch_size', type=int, default=2, help='每设备训练批次大小 (RTX 3090 优化)')
    parser.add_argument('--gradient_accumulation', type=int, default=16, help='梯度累积步数 (RTX 3090 优化)')
    parser.add_argument('--learning_rate', type=float, default=8e-5, help='学习率 (RTX 3090 优化)')
    parser.add_argument('--max_steps', type=int, default=1500, help='最大训练步数 (RTX 3090 优化)')
    parser.add_argument('--save_steps', type=int, default=200, help='保存步数')
    parser.add_argument('--logging_steps', type=int, default=20, help='日志步数')
    parser.add_argument('--warmup_steps', type=int, default=150, help='预热步数')
    parser.add_argument('--resume_from_checkpoint', type=str, default=None, help='从检查点恢复训练')
    parser.add_argument('--use_gradient_checkpointing', action='store_true', default=True, help='启用梯度检查点')
...
def main():
    ...
    print('\n' + '='*70)
    print('  Qwen3.5-7B QLoRA 微调 - RTX 3090 优化版')
    print('='*70)
    print('专门为 RTX 3090 24GB VRAM 优化')
    print('='*70)
    ...
    print(f'\n下一步:')
    print(f'  1. 使用 qwen_inference_test.py 验证模型推理')
